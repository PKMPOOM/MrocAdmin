...
import { DiscussionUserParticipateContext } from "~/src/Pages/Admin/Qualitative/Discussions/[ID]/User_participate/Subtabs/Discussion_Invite_user";
...
function Modal_Discussion_User_Invitation() {
  const { DiscussionInviteModalOpen, setDiscussionInviteModalOpen } =
    useContext(DiscussionUserParticipateContext);
  ...
  const PostNewRule = async () => {
    setLoading(true);
    console.log(CreateInvitationRequestBody);
    setLoading(false);
    ...
    //   .finally(() => {
    //     setLoading(false);
    //     setDiscussionInviteModalOpen(false);
  ...
  return (
    <Modal
      ...
      footer={null}
      open={DiscussionInviteModalOpen}
      onCancel={() => {
        setDiscussionInviteModalOpen(false);
      }}
    >
...
export default Modal_Discussion_User_Invitation;
