...
export default function BlogCard({ blog, indexData }: ContentCardProps) {
  const { isFirstIndex, isLastIndex, blogIndex, blockIndex } = indexData;
  const { AuthUser } = useAuth();
  const { token } = useThemeContext();

  const [setContentModalOpen, setIndexData, onContentSort, onContentDelete] =
  ...
  return (
    ...
    >
      ...
      >
        <div className="tw-bg-gradient-to-bl  tw-from-cyan-500 tw-to-blue-500 tw-w-full tw-flex-col tw-overflow-hidden tw-rounded-lg  tw-relative tw-h-full  tw-flex tw-gap-2 tw-justify-start tw-items-start tw-bg-center tw-bg-cover tw-bg-no-repeat ">
          {blog.image_url ? (
            <img
              style={{
                minWidth: "100%",
            ...
            <div className="tw-p-2 tw-font-semibold tw-text-lg tw-text-white">
